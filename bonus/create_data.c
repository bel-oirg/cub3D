...
/*   Updated: 2024/09/26 11:05:09 by bel-oirg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int data_creating(t_data *data)
{
	...
	data->txtr->NO = NULL;
	data->txtr->SO = NULL;
	data->txtr->door =  mlx_load_png(DOOR);
...
t_mlx *create_mlx_struct(void)
{
	...
	if (!mlx || !mlx->mlx_p || !mlx->img || !mlx->ply || !mlx->ray || !mlx->tmp || !mlx->dt 
		|| !data_creating(mlx->dt) || !mlx->tmp || !mlx->sprite || !mlx->dt->txtr->door)
		exit(before_exit(mlx,1));
	// int i;
	// i = -1;
	// while(++i <= SP_FRAMES)
	// 	mlx->sprite->txt[i] = my_malloc(sizeof(mlx_texture_t) * SP_FRAMES, 1);
	mlx->mouse_hidden = 0;
	mlx->ray->distance = 0;
	mlx->ray->ray_ngl = 0;
	mlx->ray->flag = 0;
