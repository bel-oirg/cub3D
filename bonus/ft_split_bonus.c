...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/28 12:18:42 by bel-oirg          #+#    #+#             */
/*   Updated: 2024/09/29 09:46:35 by bel-oirg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	alloc_cpy(char **res, char *line, int words, int idx)
{
	...
	while (line[i] != '\0' && line[i] != '\n' && idx < words)
	{
		...
		if (i > j)
		{
			res[idx] = my_malloc(sizeof(char) * (i - j) + 1, 1);
			if (!res[idx])
				return ;
...
char	**ft_split(char *line)
{
	...
	res = my_malloc(sizeof(char *) * (words + 1), 1);
	if (!res)
		return (NULL);
	if (res > 0)
